// Pure logic, no network calls — direct TS port of _detect_company_size,
// _get_dynamic_metrics, _get_structural_signal, _generate_infrastructure_alerts,
// _score_lead. These build alerts strictly from real evidence already
// extracted by Groq/Firecrawl; never invents figures.

import type { ScoutProfile } from "./groq";
import type { FundingResult } from "./firecrawl";

export function detectCompanySize(content: string, signals: string): string {
  const contentLower = content.toLowerCase();
  const signalsLower = signals.toLowerCase();
  if (["series c", "series d", "ipo", "1000 employees", "10,000"].some((k) => contentLower.includes(k))) {
    return "enterprise";
  }
  if (["series b", "500 employees", "global team", "100+ team"].some((k) => contentLower.includes(k))) {
    return "scale";
  }
  if (["series a", "50 employees", "growing team", "hiring fast"].some((k) => contentLower.includes(k))) {
    return "growth";
  }
  if (["hiring", "recruiting", "seed", "pre-seed"].some((k) => signalsLower.includes(k))) {
    return "startup";
  }
  return "startup";
}
...
const METRICS_BY_SIZE: Record<string, Metrics> = {
...
/** Builds alerts strictly from evidence extracted by the model/real lookups.
 * No invented financial figures — if there's no real signal for a category,
 * it's skipped. If nothing at all was found, says so honestly. */
export function generateInfrastructureAlerts(profile: ScoutProfile, funding: FundingResult): Alert[] {
  const alerts: Alert[] = [];
...
export function scoreLead(profile: ScoutProfile): { score: ScoreLabel; scoreNum: number } {
  const signals = profile.signals.toLowerCase();
  const stage = profile.stage.toLowerCase();
  const pain = profile.pain.toLowerCase();
  ...
  if (["hiring", "fund", "raised", "series", "recruit", "expanding"].some((k) => signals.includes(k))) {
    scoreNum += 10;
  }

  if (["growth", "scale", "series a", "series b"].some((k) => stage.includes(k))) scoreNum += 10;
  else if (["seed", "early"].some((k) => stage.includes(k))) scoreNum += 5;

  if (["pipeline", "revenue", "sales", "acquisition", "cac", "churn"].some((k) => pain.includes(k))) {
    scoreNum += 8;
  }
  ...
  const score: ScoreLabel = scoreNum >= 75 ? "HOT" : scoreNum >= 55 ? "WARM" : "COLD";
